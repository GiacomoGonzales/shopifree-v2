...
/**
 * Admin-only endpoint that triggers the "Build store app" GitHub Actions
 * workflow for a specific store. Marks the store's build status as 'queued'
 * in Firestore so the admin UI reflects the state immediately.
 *
 * Body: { storeId: string, versionName?: string, buildNumber?: string }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const storeData = storeSnap.data() as { appConfig?: Record<string, unknown> }
    if (!storeData.appConfig) {
      return res.status(400).json({ error: 'Store has no appConfig — configure it first' })
    }

    // Fire GitHub workflow_dispatch
    ...
    console.error('trigger-app-build error:', err)
