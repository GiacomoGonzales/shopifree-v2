...
export interface StoreAppConfig {
  ...
  // Phone screenshots auto-generated by Playwright in CI for the Play Store
  // listing. Triggered from /admin/app-builds; the workflow loads the
  // storefront in a phone viewport and uploads PNGs to Firebase Storage.
  screenshots?: {
    status: 'idle' | 'queued' | 'running' | 'success' | 'failed'
    urls?: string[]              // signed download URLs in the order taken
    generatedAt?: Date
    runUrl?: string              // GitHub Actions run for log inspection
    lastError?: string
  }
}
