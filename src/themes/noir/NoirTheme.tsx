/**
 * NOIR THEME - PREMIUM - "CINEMATICO / SPOTLIGHT"
 *
 * Filosofia: Ultra dark, cinematografico, spotlight.
 * - Fondo negro puro con vignette
 * - Spotlight radial que sigue el cursor
 * - Cards glassmorphism con bordes animados
 * - 3D tilt hover
 * - Parallax hero con overlay cinematico
 * - Texto con glow pulsante
 * - Ideal para: Moda premium, relojes, vinos, productos exclusivos
 */
...
const noirTheme: ThemeConfig = {
  ...
  shadows: {
    sm: '0 0 15px rgba(201, 169, 98, 0.06)',
    md: '0 0 25px rgba(201, 169, 98, 0.1)',
    lg: '0 0 50px rgba(201, 169, 98, 0.12)',
  },
  effects: {
    cardHover: 'scale-105',
...
export default function NoirTheme({ store, products, categories, onWhatsAppClick, onProductView, onCartAdd, initialProduct }: Props) {
  ...
  // Spotlight cursor effect via ref (no React re-renders)
  const handleGlobalMouseMove = useCallback((e: React.MouseEvent) => {
    if (!spotlightRef.current) return
    const x = (e.clientX / window.innerWidth) * 100
    const y = ((e.clientY + window.scrollY) / document.documentElement.scrollHeight) * 100
    spotlightRef.current.style.background = `radial-gradient(600px circle at ${x}% ${y}%, ${gold}08, transparent 60%)`
  }, [])
  ...
  return (
    <ThemeProvider theme={noirTheme} store={store}>
      <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;500;600;700&family=Inter:wght@300;400;500;600&display=swap" rel="stylesheet" />

      <div
        className="min-h-screen relative"
        style={{ backgroundColor: '#000000', fontFamily: "'Inter', system-ui, sans-serif" }}
        onMouseMove={handleGlobalMouseMove}
      >
        {/* Spotlight effect - ref-based, no re-renders */}
        <div
          ref={spotlightRef}
          className="fixed inset-0 pointer-events-none opacity-40 hidden md:block"
        />
        ...
        <header
          ...
          style={{
            ...
            borderBottom: `1px solid rgba(201,169,98,${scrolled ? '0.12' : '0.06'})`,
          }}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center gap-3">
                {store.instagram && (
                  <a href={`https://instagram.com/${store.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 flex items-center justify-center rounded-lg transition-all hover:bg-white/5" style={{ color: '#777' }}>
                ...
                <button
                  ...
                  style={{
                    background: totalItems > 0 ? gold : 'rgba(255,255,255,0.04)',
                    color: totalItems > 0 ? '#000000' : '#F5F5F5',
                    backdropFilter: 'blur(12px)',
                    border: totalItems > 0 ? 'none' : `1px solid ${gold}20`,
                    boxShadow: totalItems > 0 ? `0 0 30px ${gold}30` : 'none',
                  }}
                >
        ...
        {(store.heroImage || store.heroImageMobile) ? (
          <section ref={heroRef} className="relative overflow-hidden">
            <div className="md:hidden relative">
              <img ref={heroImgMobileRef} src={optimizeImage(store.heroImageMobile || store.heroImage, 'hero')} alt={store.name} className="w-full h-auto max-h-[400px] object-cover" style={{ filter: 'contrast(1.1) brightness(0.9)' }} />
              <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, transparent 40%, #000000)' }} />
            </div>
            <div className="hidden md:block relative aspect-[16/5] overflow-hidden">
              <img ref={heroImgRef} src={optimizeImage(store.heroImage || store.heroImageMobile, 'hero')} alt={store.name} className="w-full h-full object-cover" style={{ transform: 'scale(1.1)', filter: 'contrast(1.1) brightness(0.9)' }} />
              <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.3) 0%, transparent 30%, transparent 60%, #000000 100%)' }} />
            </div>
          </section>
        ) : (
          <section className="py-20 md:py-32 text-center relative">
            <div className="max-w-4xl mx-auto px-6 relative">
              <h2
                className="text-4xl md:text-7xl font-semibold italic tracking-wide"
                style={{
                  fontFamily: "'Cormorant Garamond', serif",
                  color: '#F5F5F5',
                  textShadow: `0 0 60px ${gold}20`,
