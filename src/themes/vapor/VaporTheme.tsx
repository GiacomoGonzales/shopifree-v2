/**
 * VAPOR THEME - PREMIUM - "ATMOSFERICO / MISTERIOSO"
 *
 * Efecto WOW: Nubes de humo GRANDES y visibles (opacity 0.25+),
 * 5 capas de drift a diferentes velocidades, fog gradient en bottom,
...
const vaporTheme: ThemeConfig = {
  colors: {
    background: '#0A0710',
    surface: 'rgba(160, 100, 220, 0.08)',
    surfaceHover: 'rgba(160, 100, 220, 0.14)',
    text: '#EDE6F5',
    textMuted: '#8B7BA0',
    textInverted: '#0A0710',
    primary: '#C77DFF',
  ...
  shadows: {
    sm: '0 0 20px rgba(199, 125, 255, 0.10)',
    md: '0 0 35px rgba(199, 125, 255, 0.15)',
    lg: '0 0 60px rgba(199, 125, 255, 0.20)',
  },
  effects: {
    cardHover: 'scale-105',
...
export default function VaporTheme({ store, products, categories, onWhatsAppClick, onProductView, onCartAdd, initialProduct }: Props) {
  ...
  return (
    <ThemeProvider theme={vaporTheme} store={store}>
      ...
      <div className="min-h-screen relative" style={{ backgroundColor: bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
        {/* ═══ SMOKE LAYER 1 - Large purple cloud, left drift ═══ */}
        <div className="fixed inset-0 pointer-events-none" style={{
          background: `radial-gradient(ellipse 90% 70% at 15% 55%, ${purple}30 0%, transparent 65%)`,
          animation: 'vaporDriftLeft 22s ease-in-out infinite',
          willChange: 'transform',
        }} />

        {/* ═══ SMOKE LAYER 2 - Warm red cloud, right drift ═══ */}
        <div className="fixed inset-0 pointer-events-none" style={{
          background: `radial-gradient(ellipse 80% 60% at 80% 35%, ${warm}22 0%, transparent 60%)`,
          animation: 'vaporDriftRight 28s ease-in-out infinite',
          willChange: 'transform',
        }} />

        {/* ═══ SMOKE LAYER 3 - Deep purple, diagonal drift ═══ */}
        <div className="fixed inset-0 pointer-events-none" style={{
          background: `radial-gradient(ellipse 70% 80% at 50% 75%, rgba(100,40,180,0.20) 0%, transparent 55%)`,
          animation: 'vaporDriftDiag 35s ease-in-out infinite',
          willChange: 'transform',
        }} />

        {/* ═══ FOG GRADIENT at bottom ═══ */}
        <div className="fixed bottom-0 left-0 right-0 h-40 pointer-events-none" style={{
          background: `linear-gradient(180deg, transparent, ${bg}90)`,
        }} />
        ...
        <header
          ...
          style={{
            backgroundColor: scrolled ? 'rgba(10,7,16,0.88)' : 'rgba(10,7,16,0.45)',
            borderBottom: `1px solid rgba(199,125,255,${scrolled ? '0.15' : '0.06'})`,
          }}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center gap-3">
                {store.instagram && (
                  <a href={`https://instagram.com/${store.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 flex items-center justify-center rounded-lg transition-all hover:bg-white/5" style={{ color: '#8B7BA0' }}>
                ...
                <button
                  ...
                  style={{
                    background: totalItems > 0 ? `linear-gradient(135deg, ${purple}, ${warm})` : 'rgba(160,100,220,0.08)',
                    color: totalItems > 0 ? '#fff' : '#EDE6F5',
                    backdropFilter: 'blur(12px)',
                    border: totalItems > 0 ? 'none' : `1px solid rgba(199,125,255,0.18)`,
                    boxShadow: totalItems > 0 ? `0 0 25px ${purple}50, 0 0 50px ${warm}25` : 'none',
                  }}
                >
          ...
          <section className="py-28 md:py-44 text-center relative overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px]" style={{
                background: `radial-gradient(ellipse, ${purple}25 0%, ${warm}12 40%, transparent 65%)`,
