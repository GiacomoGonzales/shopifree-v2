        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center md:text-left">
              {store.logo ? (
                <img src={store.logo} alt={store.name} className="h-10 w-auto mb-4 mx-auto md:mx-0" />
              ) : (
                <h3 className="text-xl tracking-wide mb-4">{store.name}</h3>
              )}
              ...
              <div className="space-y-3">
                {store.whatsapp && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() => onWhatsAppClick?.()}
                    className="flex items-center justify-center gap-2 text-sm transition-colors hover:opacity-70"
                  >
            ...
            <div className="text-center md:text-right">
              <h4 className="font-medium mb-4 tracking-wide" style={{ color: gold }}>Ubicacion</h4>
              <div className="space-y-2 text-sm opacity-80">
                {store.location && (
                  <p>{[store.location.city, store.location.country].filter(Boolean).join(', ')}</p>
                )}
              </div>
            </div>
          </div>
