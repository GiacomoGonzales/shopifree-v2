/**
 * PRISM THEME - PREMIUM - "PRISMA / ARCOIRIS"
 *
 * Filosofia: Luz blanca refractada en arcoiris, elegante y moderno.
 * - Fondo blanco con reflejos prismaticos que se mueven
 * - Cards con bordes rainbow animados
 * - 3D tilt hover en desktop
 * - Parallax en hero
 * - Tipografia limpia y moderna
 * - Ideal para: Moda de autor, accesorios premium, tiendas de diseno
 */
...
export default function PrismTheme({ store, products, categories, onWhatsAppClick, onProductView, onCartAdd, initialProduct }: Props) {
  ...
  return (
    <ThemeProvider theme={prismTheme} store={store}>
      ...
      <div className="min-h-screen relative" style={{ backgroundColor: '#FAFAFA', fontFamily: "'Inter', system-ui, sans-serif" }}>

        {/* Prism Background - lightweight rainbow reflections */}
        <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ willChange: 'transform' }}>
          <div
            className="absolute inset-0 opacity-15"
            style={{
              background: `
                radial-gradient(ellipse 50% 40% at 20% 30%, #FF006618 0%, transparent 60%),
                radial-gradient(ellipse 40% 35% at 70% 25%, #00FF8815 0%, transparent 55%),
                radial-gradient(ellipse 45% 30% at 50% 75%, #8800FF15 0%, transparent 55%)
              `,
        ...
        {/* Header */}
        <header
          className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'backdrop-blur-2xl shadow-sm' : 'backdrop-blur-sm'}`}
          style={{
            backgroundColor: scrolled ? 'rgba(250,250,250,0.9)' : 'rgba(250,250,250,0.7)',
            borderBottom: `1px solid rgba(0,0,0,${scrolled ? '0.06' : '0.03'})`,
          }}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center gap-3">
                ...
                {showName && <h1
                  ...
                  style={{
                    ...
                    backgroundImage: 'linear-gradient(135deg, #FF0066, #FF8800, #FFDD00, #00FF88, #0088FF, #8800FF, #FF0066)',
              ...
              <div className="flex items-center gap-3">
                {store.instagram && (
                  <a href={`https://instagram.com/${store.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 flex items-center justify-center rounded-xl transition-all hover:bg-black/5" style={{ color: '#666' }}>
                ...
                <button
                  ...
                  style={{
                    background: totalItems > 0 ? '#111111' : 'rgba(255,255,255,0.8)',
                    color: totalItems > 0 ? '#FFFFFF' : '#111111',
                    backdropFilter: 'blur(12px)',
                    border: totalItems > 0 ? 'none' : '1px solid rgba(0,0,0,0.08)',
                    boxShadow: totalItems > 0 ? '0 4px 20px rgba(0,0,0,0.15)' : '0 1px 4px rgba(0,0,0,0.04)',
                  }}
                >
        ...
        {(store.heroImage || store.heroImageMobile) ? (
          ...
          <section className="py-20 md:py-32 text-center relative">
            <div className="max-w-4xl mx-auto px-6 relative">
              ...
              {store.about?.slogan && <p className="mt-5 text-lg" style={{ color: '#666' }}><EditableText path="about.slogan" value={store.about?.slogan} /></p>}
              {store.whatsapp && (
                <a href={`https://wa.me/${store.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" onClick={() => onWhatsAppClick?.()} className="inline-flex items-center gap-2 mt-8 px-6 py-3 rounded-xl font-semibold transition-all hover:scale-105" style={{ background: '#25D366', color: '#fff', boxShadow: '0 4px 20px #25D36630' }}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
              <FilterPanel
                ...
                colors={{
                  text: prismTheme.colors.text,
                  textMuted: prismTheme.colors.textMuted,
                  border: prismTheme.colors.border,
                  background: prismTheme.colors.background,
                  primary: getPrimaryColor(store) || prismTheme.colors.primary,
                  surface: prismTheme.colors.surfaceHover,
                }}
              />
              <SortDropdown
                sortBy={sortBy}
